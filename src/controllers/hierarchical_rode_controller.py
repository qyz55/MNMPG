...
class HierarchicalRODEMAC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False, need_log_p=False):
        ...
        agent_outputs, role_outputs, _, role_log_p, _, log_p  = self.forward(ep_batch, t_ep, test_mode=test_mode, t_env=t_env)
        ...
        role_avail_actions = th.gather(self.role_action_spaces.unsqueeze(0).repeat(self.n_agents * ep_batch.batch_size, 1, 1), dim=1,
        ...
        if need_log_p:
            return chosen_actions, self.selected_roles, role_avail_actions, (role_log_p, log_p)
        else:
    ...
    def forward(self, ep_batch, t, test_mode=False, t_env=None):
        ...
        # select roles
        self.role_skill = None
        self.role_log_p = None
        if self.args.meta_role:
            self.role_hidden_states, self.role_skill, self.role_log_p = self.role_agent(agent_inputs, self.role_hidden_states)
        else:
        ...
        self.hidden_states, self.skill, self.log_p = self.agent(agent_inputs, self.hidden_states)
        roles_q = []
        ...
        return agent_outs.view(ep_batch.batch_size, self.n_agents, -1), \
            (None if role_outputs is None else role_outputs.view(ep_batch.batch_size, self.n_agents, -1)), \
                self.role_skill, self.role_log_p, self.skill, self.log_p
    ...
    def _build_agents(self, input_shape):
        ...
        if self.args.agent == "hierarchical_rode" and not self.args.meta_role:
            self.role_agent = agent_REGISTRY["rode"](input_shape, self.args)
        else:
    ...
    def update_role_action_spaces(self):
        ...
        print('<<< Original Action Spaces', spaces)
