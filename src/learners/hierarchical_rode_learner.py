...
class HierarchicalRODELearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.role_mixer is not None:
            ...
            elif args.role_mixer == "qmix" or (args.role_mixer == "hqmix" and not args.meta_role):
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, chosen_index=0):
        ...
        for t in range(batch.max_seq_length):
            agent_outs, role_outs, role_skills, _, skills, _ = self.mac.forward(batch, t=t)
            mac_out.append(agent_outs)
            skill_out.append(skills)
            if t % self.role_interval == 0 and t < batch.max_seq_length - 1:
                role_out.append(role_outs)
                role_skill_out.append(role_skills)
        mac_out = th.stack(mac_out, dim=1)  # Concat over time
        role_out = th.stack(role_out, dim=1)  # Concat over time
        if self.args.meta_role:
            role_skill_out = th.stack(role_skill_out, dim=1)
        ...
        target_mac_out = []
        target_role_out = []
        target_role_skill_out = []
        target_skill_out = []
        self.target_mac.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            target_agent_outs, target_role_outs, target_role_skills, _, target_skills, _ = self.target_mac.forward(batch, t=t)
            target_mac_out.append(target_agent_outs)
            target_skill_out.append(target_skills)
            if t % self.role_interval == 0 and t < batch.max_seq_length - 1:
                target_role_out.append(target_role_outs)
                target_role_skill_out.append(target_role_skills)

        target_role_out.append(th.zeros(batch.batch_size, self.n_agents, self.mac.n_roles).to(self.device))
        target_role_skill_out.append(th.zeros(batch.batch_size, self.mixer.latent_dim).to(self.device))
        ...
        if self.args.meta_role:
            target_role_skill_out = th.stack(target_role_skill_out[1:], dim=1)
        ...
        if self.role_mixer is not None:
            ...
            if self.args.meta_role:
                chosen_role_qvals = self.role_mixer(chosen_role_qvals, role_states, role_skill_out)
            else:
                chosen_role_qvals = self.role_mixer(chosen_role_qvals, role_states)
            role_states = th.cat([role_states[:, 1:], role_states[:, 0:1]], dim=1)
            if self.args.meta_role:
                target_role_max_qvals = self.target_role_mixer(target_role_max_qvals, role_states, target_role_skill_out)
            else:
