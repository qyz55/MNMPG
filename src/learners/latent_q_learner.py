...
class LatentQLearner(QLearner):
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, chosen_index=0):
        ...
        rewards = batch["reward"][:, :-1] #[32,180,1]
        actions = batch["actions"][:, :-1] #[32,180,10,1]
        terminated = batch["terminated"][:, :-1].float() #[32,180,1]
        mask = batch["filled"][:, :-1].float() 
        mask[:, 1:] = mask[:, 1:] * (1 - terminated[:, :-1])  #[32,180,1]
        avail_actions = batch["avail_actions"] #[32,181,10,18]
        ...
        for t in range(batch.max_seq_length):
            agent_outs, loss_, dis_loss_, ce_loss_ = self.mac.forward(batch, t=t, t_glob=t_env, train_mode=True)  # (bs,n,n_actions),(bs,n,latent_dim)[32,10,18]
            reg_loss += loss_
            dis_loss += dis_loss_
            ce_loss += ce_loss_
