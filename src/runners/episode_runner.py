...
class EpisodeRunner:
    ...
    def run(self, test_mode=False, meta_mode=False, use_rode=False):
        ...
        while not terminated:
            ...
            if meta_mode:
                actions, log_p = chosen_mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, test_mode=test_mode, need_log_p=True)
                log_ps.append(log_p)
            else:
        ...
        if meta_mode:
            actions, log_p = chosen_mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, test_mode=test_mode, need_log_p=True)
            log_ps.append(log_p)
            all_log_p = th.cat([it.unsqueeze(1) for it in log_ps], dim=1) #[8*max_ep_len*3]
