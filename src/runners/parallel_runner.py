...
class ParallelRunner:
    ...
    def run(self, test_mode=False, meta_mode=False):
        self.reset()
        ...
        if self.args.mac == "separate_mac" or self.args.mac == "hierarchical_mac":
        ...
        while True:
            ...
            if meta_mode:
                actions, log_p = chosen_mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode, need_log_p=True)
                log_ps.append(log_p)
            else:
                actions = chosen_mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode)
